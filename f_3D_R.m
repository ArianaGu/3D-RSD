function u_volume = f_3D_R(u_total, M, N, lambda_loop, weight, sample_spacing,camera_pos,laser_pos, R)
...
for spectrum_index = 1 : length(lambda_loop)
    ...
    u_mid = propRSD_3D(u1, M, N, camera_pos,sample_spacing,lambda, depth1, R);
...
function[u2] = propRSD_3D(u1,M, N, camera_pos,sample_spacing,lambda, z, R)
    ...
    for x = 1:M
        for y = 1:N
            ...
            index = find(r < R);
            eff_r = r(index);
            RSD = exp(1j * 2*pi/lambda * eff_r)./eff_r;
            u2(x,y) = sum(u1(index).*RSD, 'all');
        end
    end
end
